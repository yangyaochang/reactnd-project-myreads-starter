...
export default class Shelves extends Component {
    ...
    moveBook(preType, targetType, id) {
        this.setState(preState => {
            let book = this.deleteBook(preState[preType], id) 
            this.pushBook(preState[targetType], book)
            return preState
        })
    }

    deleteBook(arrOfBooks, id) {
        let book
        for (let i = 0; i < arrOfBooks.length; i++) {
            if (arrOfBooks[i].id === id) {
                book = arrOfBooks.splice(i, 1)[0]
                break
            }
        }
        return book
    ...
    render() {
        ...
        return (
            <div>
                <Bookshelf type = {'Currently Reading'} books = {currentlyReading} moveBook = {this.moveBook.bind(this)}/>
                <Bookshelf type = {'Want to Read'} books = {wantToRead} moveBook = {this.moveBook.bind(this)}/>
                <Bookshelf type = {'Read'} books = {read} moveBook = {this.moveBook.bind(this)}/>
            </div>
